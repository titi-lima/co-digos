...
char **criaMatriz(char **tab) {
    ...
    for (aux = 0; aux < 8; ++aux) {
        ...
        for (aux2 = 0; aux2 < n; ++aux2) { // zera a matriz, util depois para verificar se o movimento eh valido.
            tab[aux][aux2] = '0';
        }
    }
...
int movePeao(char **tab, char jogada[7]) {
    int kx = (jogada[1]-97), jx=jogada[4]-97; // pega uma letra minuscula e transforma em numero, tal que a=0 e z=25. util p/ verificar na matriz
    int ky = (jogada[2]-49), jy=(jogada[5]-49) // similarmente, transforma o caracter do numero em numero p/ checagem na matriz.
    if(tab[kx][ky] == 'P') { // de fato, tem um peao de onde o jogador quer mover
        if(jx == kx+2 && jy == ky+2) { // movemento diagonal do peao
            if tab[jx+1][jy1+1] != '0' { // tem uma peca no caminho, condicao necessaria para esse movimento
