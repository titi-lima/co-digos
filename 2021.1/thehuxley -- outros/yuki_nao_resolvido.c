    ...
    for (Q = 0; Q < q; ++Q) {
        ...
        for (C = 0; C < c; ++C) {
            for (M = 0; M < m * n; ++M) {
                ...
                if ((strncasecmp(&nomes[C][0], &caca[0][M], q) == 0) && (M % m < (M + q) % m && q < m)) {
                    ++cont;
                }
            }
        }
        for (M = 0; M < m * n; ++M) {
            if ((strncasecmp(&caca[0][M], elaaaa, 4) == 0) && (M % m < (M + 4) % m && 4 < m)) {
                ++cont;
            }
        }
        //printf("M:%d\nN:%d\n", m, n);
        for (N = 0; N < n; ++N) {
            invertida[N][0] = '\0';
            for (M = 0; M < m; ++M) {
                invertida[N][M] = caca[n - N - 1][m - M - 1];
                //printf("M:%d", M);
            }
        }
        for (C = 0; C < c; ++C) {
        ...
        for (M = 0; M < m; ++M) {
            ...
            for (N = 0; N < n; ++N) {
                transpinvertida[M][N] = transposta[m - M - 1][n - N - 1];
            }
        }
        //printf("AQUI: %s\n", &transpinvertida[0][0]);
        for (C = 0; C < c; ++C) {
            for (M = 0; M < m; ++M) {
                for (N = 0; N < n; ++N) {
                    ...
                    if ((strncasecmp(&nomes[C][0], &transpinvertida[M][N], q) == 0)) {
                        ++cont;
                        //printf("%d cont em transpinvertida\n", cont);
                        //printf("%s\n", &nomes[C][0]);
                    }
                }
            }
